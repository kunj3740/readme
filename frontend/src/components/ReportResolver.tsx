...
export const AdminReportedBlogs = () => {
  const [isDarkMode, setIsDarkMode] = useState(true);
  const { loading , setLoading  } = useContextedBlogs();
  const [reports, setReports] = useState<ReportedBlog[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterBy, setFilterBy] = useState('all');
  ...
  const handleDeleteBlog = async (blogId: number) => {
    try {
      ...
      setLoading(!loading);
      // Remove any reports related to this blog
      setReports(reports.filter(report => report.blog.id !== blogId));
              ...
              >
                <div className="flex flex-wrap justify-between items-start mb-4">
                  ...
                  <div className="flex items-center space-x-2 mt-2 sm:mt-0">
                    ...
                    {/* Delete Blog Button */}
                    <button
                      onClick={() => handleDeleteBlog(report.blog.id)}
                      disabled={deletingBlogId === report.blog.id}
                      className={`p-2 rounded-md flex items-center ${
                        isDarkMode 
